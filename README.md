...
- `server/` — Node.js + TypeScript + Socket.IO 백엔드 (방 1개, 게임 상태 총괄)
...
cd app && npm install && npm run web      # http://localhost:8082 (또는 npm start 로 Expo Go 연동)
```
...
3. 루트의 `render.yaml`이 자동으로 `server/`를 Node 웹 서비스(무료 플랜)로 배포
